...
<meta itemprop="description" content="Tutorial for getting started with GoJS" />
<meta property="og:description" content="Tutorial for getting started with GoJS" />
<meta name="twitter:description" content="Tutorial for getting started with GoJS" />
...
<meta property="og:url" content="https://gojs.net/latest/learn/" />
<meta property="twitter:url" content="https://gojs.net/latest/learn/" />
...
<p>
  First, let's load the <b>GoJS</b> library. If you have the library downloaded to your machine:
</p>
...
<pre class="lang-html"><code>
&lt;script src="https://cdn.jsdelivr.net/npm/gojs/release/go-debug.js"&gt;&lt;/script&gt;</code></pre>

<p>
  If you are using a package manager such as <a target="_blank" href="https://npmjs.com/package/gojs">npm</a>
  and you are using <code>import</code> statements, it would just be:
</p>

<pre class="lang-js"><code>
  // automatically chooses to use go-debug.js when developing and go.js when deploying
  import * as go from "gojs";
</code></pre>

<h3>The Hosting Div</h3>
<p>
  Each <b>GoJS</b> diagram is hosted by an HTML <code>&lt;div&gt;</code> element in your HTML page that you give a size:
</p>
...
<p>
  Notice that <code>go</code> is the JavaScript "namespace" in which all <b>GoJS</b> types reside.
  In this documentation all code uses of <b>GoJS</b> classes such as Diagram or Node or Panel or
...
<p>
  ...
  <b>GoJS</b> has a model-view architecture, where Models hold the data (arrays of JavaScript objects) that describe nodes and links,
...
<ul>
  ...
  <li>
    ...
    (On a Mac you can use the Command key as the modifier for all commands,
    but you must use Alt as the modifier for a copying drag-and-drop.)
  </li>
  <li>
...
<ul>
  ...
  <li><a href="../intro/panels.html">Panel</a>, containers to hold a collection of other objects that can be positioned and sized in different manners according
    to the type of the Panel (like tables, vertical stacks, and stretching containers)</li>
</ul>

<p>
  All of these building blocks are derived from the <a href="../api/symbols/GraphObject.html">GraphObject</a> abstract class,
  so we casually refer to them as GraphObjects or objects or elements.
  Note that a GraphObject is <em>not</em> an HTML DOM element, so there is much less overhead in creating or modifying such objects.
</p>
...
<p>
  The default Node template is simple: a Node that contains one TextBlock.
  This was shown in the previous example.
  There is a data binding between a TextBlock's <code>text</code> property and the model data's <code>text</code> or <code>key</code> property.
...
<p>
  The nesting of GraphObjects within Panels can be arbitrarily deep, and every class has its own unique set of properties to utilize,
  ...
  new go.Node("Horizontal", {
    ...
    .add(
      ...
      new go.TextBlock("Default Text", {  // the initial value for TextBlock.text
          // some room around the text, a white stroke, and a larger font:
...
myDiagram.model = new go.Model(
  [ // note that each node data object holds whatever properties it needs;
    ...
    // because in our template above, we have defined bindings that expect them
...
<p>
  ...
  The "empty" node data in this example is used to show that node templates can work perfectly well
  without any of the properties on the bound data.
</p>
...
<p>
  ...
  So we will create a complete organization chart diagram by adding some Links to show the relationship between
  individual nodes and a Layout to automatically position the nodes.
</p>
...
<p>
  ...
  Instead of maintaining a separate array of link data, the links in a tree model are created by specifying a "parent" for a node data.
...
<p>
  ...
  Our organizational diagram is a simple hierarchical tree-like structure, so we will choose TreeModel for this example.
</p>
...
<p>
  Diagrams have a default layout which takes all nodes that do not have a location and gives them locations, arranging them in a grid.
...
<p>
  ...
  TreeLayout defaults to flowing from left to right, so to get it to flow from top to bottom (as is common in organizational diagrams),
  we will set the <code>angle</code> property to 90.
</p>

<p>
  Using layouts in <b>GoJS</b> is usually simple.
  Each kind of layout has a number of properties that affect the results.
  There are samples for each layout (like <a href="../samples/tLayout.html">TreeLayout Demo</a>) that showcase its properties.
</p>
...
<p><b>GoJS</b> has many other layouts, which you can read about <a href="../intro/layouts.html">here</a>.</p>

<p>Adding the layout to the diagram and model so far, we can see our results:</p>
...
<p>
  ...
  The main element of a Link is the Link's shape, and must be a Shape that will have its geometry computed dynamically by <b>GoJS</b>.
  Our link is going to consist of just this shape, with its stroke a little thicker than normal and dark gray instead of black.
...
<p>
  Combining our Link template with our Node template, TreeModel, and TreeLayout, we finally have a full organization diagram.
...
<p>
  ...
  <a href="graphObject.html">GraphObject Manipulation</a> tutorial and the <a href="interactivity.html">Interactivity</a> tutorial.
