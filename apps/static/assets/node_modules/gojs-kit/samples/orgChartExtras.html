...
<script id="code">
  function init() {
    ...
    function textStyle(tb) {
      tb.font = '9pt sans-serif';
      tb.stroke = 'white';
    }

    function nodeEvents(node) {
      node.doubleClick = nodeDoubleClick;
        ...
        .add(
          ...
          new go.Panel('Table', {
            ...
            .add(
              ...
              new go.TextBlock('ID: ')
                .apply(textStyle)
                .set({ row: 2, column: 0 }), // the ID and the boss
              new go.TextBlock()
                .apply(textStyle)
                .set({ row: 2, column: 1 })
                .bind('text', 'key'),
              new go.TextBlock('Boss: ')
                .apply(textStyle)
                .set({ row: 2, column: 3 }),
              new go.TextBlock()
                .apply(textStyle)
    ...
    myDiagram.linkTemplate =
      new go.Link({ routing: go.Routing.Orthogonal, corner: 5 })
        .add(new go.Shape({ strokeWidth: 2 })); // the link shape
