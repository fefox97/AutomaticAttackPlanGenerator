...
<meta name="description" content="Demonstrates a customized CommandHandler to update a live HTML element for accessibility." />
<meta itemprop="description" content="Demonstrates a customized CommandHandler to update a live HTML element for accessibility." />
<meta property="og:description" content="Demonstrates a customized CommandHandler to update a live HTML element for accessibility." />
<meta name="twitter:description" content="Demonstrates a customized CommandHandler to update a live HTML element for accessibility." />
...
<meta itemprop="name" content="Accessible Diagram Example" />
<meta property="og:title" content="Accessible Diagram Example" />
<meta name="twitter:title" content="Accessible Diagram Example" />
...
<meta property="twitter:domain" content="gojs.net" />
    ...
    <title>
      Accessible Diagram Example | GoJS Diagramming Library
    </title>
  </head>
...
<script id="code">
  function init() {
    myDiagram = new go.Diagram('myDiagramDiv', {
      ...
      'commandHandler.liveElementId': 'myLiveRegion',
      // customize what is read by a screen reader
      'commandHandler.focusChanged': (oldobj, newobj, cmd) => {
        const region = document.getElementById(cmd.liveElementId);
        if (region) region.textContent = describe(oldobj, newobj, cmd);
      }
    });

    function describe(oldobj, newobj, cmd) {
      let str = '';
      if (!newobj) {
        str = 'Organization Chart for the Example Company';  // when no focus GraphObject -- describe the whole Diagram
      } else if (newobj instanceof go.Link) {
        str = cmd.describe(oldobj, newobj, cmd);
      } else if (newobj instanceof go.Node) {
        const d = newobj.data;  // custom string including most important data
        str = `Employee ${d.name} ${d.title} ${d.dept}`;
        if (newobj.findNodesOutOf().count > 0) {
          let emps = '';
          newobj.findNodesOutOf().each(to => {
            if (emps) emps += ', ';
            emps += to.data.name;
          });
          if (emps) str += '; Employees: ' + emps;
        }
      } else {
        str = cmd.describe(oldobj, newobj, cmd)
        const tt = toolTipTextConverter(newobj);
        if (tt) str += ' ' + tt;
      }
      return str;
    }
    ...
    function addEmployee(node) {
      ...
      myDiagram.commandHandler.focus = newnode;
    }
  ...
  function enableKeyboard() {
    window.removeEventListener('keydown', enableKeyboard);
    myDiagram.commandHandler.isFocusEnabled = true;
    myDiagram.commandHandler.isVirtualPointerEnabled = true;
  }
...
<div id="sample">
  ...
  <p>
    This sample is a copy of the <a href="./orgChartEditor.html">Org Chart Editor</a> sample,
    but has better support for accessibility.  Diff the two files to see the enhancements.
  </p>
  <p>
    First, this sample sets <a>CommandHandler.isFocusEnabled</a> and <a>CommandHandler.isVirtualPointerEnabled</a> to true
    when the user tabs into the diagram,
    so that the user does not have to type <code>Ctrl-Alt-Enter</code>, which they will probably not know about.
  </p>
  <p>
    Second, this sample provides a custom <a>CommandHandler.focusChanged</a> event handler that updates a Div named "myLiveRegion".
    As navigation focus changes within the diagram, the text of the live region is set to a description of the current
    <a>CommandHandler.focus</a> GraphObject.
    Furthermore, when a new employee node is added, it becomes the new focus.
  </p>
  <p>
    Each diagram should be customized thoroughly for your particular application, where you know what your users want or need to know.
    Note how in this sample it talks about employees instead of the more generic "Node"s and "Link"s in the
    <a href="./orgChartEditor.html">Org Chart Editor</a> sample.
    Also, this sample makes sure there is a textual description of the "TreeExpanderButton" and the button to call AddEmployee.
    In this case it just uses the same text that is used in a "ToolTip" for each button.
  </p>
  <p>
    For another sample demonstrating screen-reader assistance, see <a href="./IVRtree.html">IVR Tree</a> sample.
    A more sophisticated accessibility customization could provide information for where the user could change focus to, and how to do so.
  </p>
  <div>
    <div>
