...
<script id="code">
  ...
  function init() {
    ...
    myDiagram = new go.Diagram('myDiagramDiv', { // must name or refer to the DIV HTML element
      ...
      "InitialLayoutCompleted": e => {
        document.getElementById('myStatus').textContent = 'Initial Layout Completed';
      },
      nodeTemplate:
        new go.Node('Auto', {
            click: nodeClicked,
              ...
              go.GraphObject.build('ContextMenu')
                .add(
                  go.GraphObject.build('ContextMenuButton', {
                    ...
                    .add(new go.TextBlock('Properties'))
                )
          })
          .add(
  ...
  // These are the HTML button event handlers.
  ...
  function clickLambda() {
    ...
    // Alternatively, if the GraphObject.click property is non null, you could just call it:
    // const clickfunc = lambda.click;
    // if (clickfunc !== null) {
    //   const ie = new go.InputEvent();
    //   ie.diagram = myDiagram;
    //   ie.documentPoint = new go.Point(loc.x + 10, loc.y + 10);
    //   ie.viewPoint = myDiagram.transformDocToView(ie.documentPoint);
    //   clickfunc(ie, lambda);
    // }
  }
  ...
  function doubleClickLambda() {
    ...
    // Alternatively, if the GraphObject.doubleClick property is non null, you could just call it:
    // const clickfunc = lambda.doubleClick;
    // if (clickfunc !== null) {
    //   const ie = new go.InputEvent();
    //   ie.diagram = myDiagram;
    //   ie.documentPoint = new go.Point(loc.x + 10, loc.y + 10);
  ...
  function linkNodes() {
    var alpha = myDiagram.findNodeForKey('Alpha');
    if (alpha === null) return;
    var ptA = alpha.getDocumentPoint(go.Spot.Bottom);
    var alpha2 = myDiagram.findNodeForKey('Alpha2');
    if (alpha2 === null) return;
    var ptB = alpha.actualBounds.center;

    // Simulate dragging from the inside edge of the Alpha node, to start drawing a new link.
    myRobot.mouseDown(ptA.x, ptA.y-2, 0);
    myRobot.mouseMove(ptA.x+10, ptA.y+10, 100);
    myRobot.mouseMove((ptA.x+ptB.x)/2, (ptA.y+ptB.y)/2, 200);
    myRobot.mouseUp(ptB.x, ptB.y, 250);
    // Now should have drawn a new Link

    // Alternatively, you could draw a new Link via:
    // const tool = myDiagram.toolManager.linkingTool;
    // tool.insertLink(alpha, null, alpha2, null);
  }
  ...
  function dragSelectNodes() {
    ...
    myRobot.mouseMove(area.x+10, area.y+10, 300);
    myRobot.mouseMove(area.right-10, area.bottom-10, 350);
    myRobot.mouseUp(area.right, area.bottom, 500);
    // Now should have selected both "Alpha" and "Alpha2", and any Link between them, using the DragSelectingTool.

    // Alternatively you could select the Nodes programmatically:
    // alpha.isSelected = true;
    // alpha2.isSelected = true;

    // Or you could select all Parts in the rectangular AREA:
    //myDiagram.selectCollection(myDiagram.findPartsIn(area));
  }
  ...
  function pan() {
    ...
    // Alternatively, you could scroll programmatically:
    // const newpos = myDiagram.position.copy();
    // newpos.offset(-80, -40);
    // myDiagram.position = newpos;
  }
...
<div id="sample">
  <div style="width: 100%; display: flex; justify-content: space-between">
    <div id="myPaletteDiv" style="width: 80px; height: 300px; margin-right: 2px; border: solid 1px black"></div>
    <div id="myDiagramDiv" style="flex-grow: 1; height: 300px; border: solid 1px black"></div>
  </div>
  ...
  Message: <span id="myStatus" style="color: green; font:bold 10pt sans-serif;">(none)</span>
  <p>
    ...
    <p>
      ...
      <button onclick="linkNodes()">Link Nodes</button><br />
