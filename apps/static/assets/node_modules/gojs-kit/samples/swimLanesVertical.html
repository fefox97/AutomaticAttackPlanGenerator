...
<meta itemprop="description" content="Vertical swim lanes and pools with collapsible lanes and limited dragging." />
<meta property="og:description" content="Vertical swim lanes and pools with collapsible lanes and limited dragging." />
<meta name="twitter:description" content="Vertical swim lanes and pools with collapsible lanes and limited dragging." />
...
<meta itemprop="name" content="Vertical, Collapsible, Resizable, Re-orderable Swim Lanes for Flow Diagram" />
<meta property="og:title" content="Vertical, Collapsible, Resizable, Re-orderable Swim Lanes for Flow Diagram" />
<meta name="twitter:title" content="Vertical, Collapsible, Resizable, Re-orderable Swim Lanes for Flow Diagram" />


<meta property="og:image" content="https://gojs.net/latest/assets/images/screenshots/swimlanesvertical.png" />
<meta itemprop="image" content="https://gojs.net/latest/assets/images/screenshots/swimlanesvertical.png" />
<meta name="twitter:image" content="https://gojs.net/latest/assets/images/screenshots/swimlanesvertical.png" />

<meta property="og:url" content="https://gojs.net/latest/samples/swimLanesVertical.html" />
<meta property="twitter:url" content="https://gojs.net/latest/samples/swimLanesVertical.html" />
...
<meta property="twitter:domain" content="gojs.net" />
    ...
    <title>
      Vertical, Collapsible, Resizable, Re-orderable Swim Lanes for Flow Diagram | GoJS Diagramming Library
    </title>
  </head>
...
<script id="code">
  ...
  function init() {
    ...
    function groupStyle(grp) {
      ...
      grp.minLocation = new go.Point(-Infinity, NaN); // only allow horizontal movement
    ...
    myDiagram.groupTemplateMap.add('Lane',
      new go.Group('Vertical')
        ...
        .set({
          ...
          layout: new go.LayeredDigraphLayout({ // automatically lay out the lane's subgraph
        ...
        .add(
              ...
              new go.Panel('Horizontal') // this is hidden when the swimlane is collapsed
                ...
                .add(
                  new go.Shape('Diamond', {
                    ...
                    .bind('fill', 'color'),
                  new go.TextBlock({ // the lane label
          ...
          new go.Panel('Auto') // the lane consisting of a background Shape and a Placeholder representing the subgraph
            .add(
              ...
              new go.TextBlock({ // this TextBlock is only seen when the swimlane is collapsed
                  name: 'LABEL',
                  font: 'bold 13pt sans-serif',
                  editable: true,
                  angle: 90,
...
<div id="sample">
  ...
  <p>
    In this design each swimlane is implemented by a <a>Group</a>, and all lanes are inside a "Pool" Group. Each lane Group has its own <a>Group.layout</a>,
    which in this case is a <a>LayeredDigraphLayout</a>. Each pool Group has its own custom <a>GridLayout</a> that arranges all of its lanes in a vertical
    stack. That custom layout makes sure all of the pool's lanes have the same length. If you don't want each lane/group to have its own layout, you could use
    set the lane group's <a>Group.layout</a> to null and set the pool group's <a>Group.layout</a> to an instance of <a>SwimLaneLayout</a>, shown at
    <a href="../samples/SwimLaneLayout.html">Swim Lane Layout</a>.
  </p>
  <p>
    When dragging nodes note that the nodes are limited to stay within the lanes. This is implemented by a custom <a>Part.dragComputation</a> function, here
    named <b>stayInGroup</b>. Hold down the Shift key while dragging simple nodes to move the selection to another lane. Lane groups cannot be moved between
    pool groups.
  </p>
  <p>
    A Group (i.e. swimlane) is movable but not copyable. When the user moves a lane up or down the lanes automatically re-order. You can prevent lanes from
    being moved and thus re-ordered by setting Group.movable to false.
  </p>
  <p>Each Group is collapsible. The previous breadth of that lane is saved in the savedBreadth property, to be restored when expanded.</p>
  <p>
    When a Group/lane is selected, its custom <a>Part.resizeAdornmentTemplate</a> gives it a broad resize handle at the bottom of the Group and a broad resize
    handle at the right side of the Group. This allows the user to resize the "breadth" of the selected lane as well as the "length" of all of the lanes.
    However, the custom <a>ResizingTool</a> prevents the lane from being too narrow to hold the <a>Group.placeholder</a> that represents the subgraph, and it
    prevents the lane from being too short to hold any of the contents of the lanes. Each Group/lane is also has a <a>GraphObject.minSize</a> to keep it from
    being too narrow even if there are no member <a>Part</a>s at all.
  </p>
  <p>A different sample has its swim lanes horizontally oriented: <a href="swimLanes.html">Swim Lanes (horizontal)</a>.</p>
