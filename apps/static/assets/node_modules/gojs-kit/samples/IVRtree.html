...
<script id="code">
  function init() {
    myDiagram = new go.Diagram('myDiagramDiv', {
      ...
      'commandHandler.liveElementId': 'screenreader',
      'commandHandler.focusChanged': (oldfocus, newfocus, cmd) => updateFocus(newfocus, oldfocus),
      'TreeCollapsed': e => updateFocus(e.subject.first().findObject('TreeExpander')),
      'TreeExpanded': e => updateFocus(e.subject.first().findObject('TreeExpander')),
      'Changed': e => { // there's no DiagramEvent for 'PanelExpanderButton' actions
        if (e.change === go.ChangeType.Property && e.object.name === 'COLLAPSIBLE' && e.propertyName === 'visible') {
          updateFocus(e.object.part.findObject('PanelExpander'));
        }
      },
    ...
    function updateFocus(newfocus, oldfocus) {
      const cmd = myDiagram.commandHandler;
      if (!newfocus) newfocus = cmd.focus;
      if (!oldfocus) oldfocus = null;
      let msg = '';
      if (newfocus instanceof go.Link) {
        msg = cmd.describe(oldfocus, newfocus);
      } else {
        const node = (newfocus && !(newfocus instanceof go.Node) && newfocus.part instanceof go.Node) ? newfocus.part : null;
        if (newfocus instanceof go.Node) {
          msg = newfocus.data.question || newfocus.data.text;
          if (Array.isArray(newfocus.data.actions) && newfocus.data.actions.length > 0) {
            msg += '. Choices: ' + newfocus.data.actions.map(act => act.text).join(', ');
          }
        } else if (newfocus && newfocus.name === 'PanelExpander') {
          msg = 'Button: ' + (newfocus.part.findObject('COLLAPSIBLE').visible ? 'Collapse' : 'Expand') + ' list of choices';
          if (node) msg += '. Type Escape to focus on ' + (node.data.question || node.data.text);
        } else if (newfocus && newfocus.name === 'TreeExpander') {
          msg = 'Button: ' + (newfocus.part.isTreeExpanded ? 'Collapse' : 'Expand') + ' subtree of choices or people';
          if (node) msg += '. Type Escape to focus on ' + (node.data.question || node.data.text);
        }
      }
      // normal use of the <span> holding content that is read aloud:
      const screenreader = document.getElementById('screenreader');
      if (screenreader) screenreader.textContent = msg;
    }
    ...
    go.Shape.defineFigureGenerator('Van', (shape, w, h) => {
    ...
    go.Shape.defineFigureGenerator('SUV', (shape, w, h) => {
      var geo = go.Geometry.parse(
    ...
    go.Shape.defineFigureGenerator('Hammer', (shape, w, h) => {
      var geo = go.Geometry.parse(
    ...
    var actionTemplate =
      new go.Panel('TableRow')
        .add(
          new go.Shape({ column: 0, width: 20, height: 20, fill: null })
            .bind('figure')
            .theme('stroke', 'text'),
          new go.TextBlock({ column: 1, font: '11pt Verdana, sans-serif' })
            .bind('text')
            .theme('stroke', 'text'),
    ...
    myDiagram.nodeTemplate = // the default node template
      new go.Node('Vertical', {
          ...
          locationSpot: go.Spot.Top  // centered vertically, aligned at top horizontally
        })
        ...
        .add(
          ...
          new go.Panel({ // this is underneath the "BODY"
            ...
            .add(go.GraphObject.build('TreeExpanderButton', { name: 'TreeExpander' }))
        );

    // define a second kind of Node:
    myDiagram.nodeTemplateMap.add('Terminal',
      new go.Node('Spot', { locationSpot: go.Spot.Top })
        .add(
          new go.Shape('Circle', { width: 55, height: 55, strokeWidth: 0 })
            .theme('fill', 'terminal'),
          new go.TextBlock({ font: '10pt Verdana, sans-serif', stroke: 'white' })
            .bind('text')
        )
    );
    ...
    myDiagram.linkTemplate =
      new go.Link({
          ...
          deletable: false, toShortLength: 4
        })
        .add(
...
<div id="sample">
  ...
  Screen Reader: <span id="screenreader"></span>
  <p>
  ...
  <p>
    Once <a>CommandHandler.isFocusEnabled</a> has been set to true, the root node becomes the GoJS <a>CommandHandler.focus</a>,
    as shown by a double-colored dashed box around the node.
    This allows the user to use the arrow keys to change focus, <code>Space</code> to select the focused Part,
    and <code>Enter</code> to go into Nodes and click buttons such as the "PanelExpanderButton" or "TreeExpanderButton".
    Read more about <a href="../learn/interactivity.html#FocusKeyboardControlTools">Keyboard Control</a>.
  </p>
  <p>
    Once <a>CommandHandler.isVirtualPointerEnabled</a> has also been set to true, then when the user holds down the <code>Shift</code> key,
    it shows a virtual pointer that allows the user to use <code>Shift-Enter</code> and <code>Shift-arrow</code> keys to drag nodes.
    The user can always toggle "keyboard control mode" using the <code>Ctrl-Alt-Enter</code> keyboard command.
  </p>
  <p>
    For another sample demonstrating screen-reader assistance, see <a href="./Accessibility.html">Accessibility</a> sample.
  </p>
  <p>
    The dark and light themes are controlled using the <a>ThemeManager</a>.
  </p>
</div>
