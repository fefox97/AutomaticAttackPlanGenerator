...
<script id="code">
  function init() {
    ...
    function textStyle(tb, big) {
      tb.stroke = 'white';
      tb.font = big ? '16pt sans-serif' : '10pt sans-serif';
      tb.alignment = go.Spot.Left;
    }

    function borderStyle(shp) {
      shp.fill = 'cornflowerblue';
      shp.stroke = 'blue';
      shp.strokeWidth = 2;
    }

    var aggregatedValue =
      new go.Panel('Auto', { margin: 5 })
        .add(
          new go.Shape().apply(borderStyle),
          new go.TextBlock({ margin: 10, width: 50 })
            .apply(textStyle)
            .bind('text', '')
          ...
          new go.Panel('Table', {
            ...
            .add(
              new go.TextBlock({ row: 0 })
                .apply(textStyle)
                .bind('text', 'header'),
              new go.Panel('Horizontal', { row: 1, itemTemplate: aggregatedValue })
      ...
      new go.Panel('Auto', {
        ...
        .add(
          ...
          new go.Panel('Vertical', {
            ...
            .add(
              new go.TextBlock({ maxSize: new go.Size(70, NaN) })
                .apply(textStyle)
                .bind('text', 'header'),
    ...
    var checkBoxTemplate =
      go.GraphObject.build('CheckBox', {
          'Button.width': 14,
          'Button.height': 14
        }, 'checked') // checked refers to the model data property name
        .add(
          new go.TextBlock()
            .apply(textStyle)
            .bind('text', 'label')
        ...
        .add(
          new go.Panel('Auto', {
            ...
            .add(
              new go.Shape()
                .apply(borderStyle),
              new go.TextBlock({ margin: 10, alignment: go.Spot.Center })
                .apply(textStyle, true)
                .bind('text', 'title')
            ),
          ...
          new go.Panel('Vertical', {
            ...
            .add(
              new go.TextBlock()
                .apply(textStyle, true)
                .bind('text', 'choices'),
              new go.Panel('Vertical', { itemTemplate: checkBoxTemplate })
                .bind('itemArray', 'checkBoxes')
            )
        )
    );
