...
<script id="code">
  function init() {
    ...
    for (let i = 0; i < 10; i++) data.push({ layer: 1, color: 'tomato', width: 50 + Math.random() * 50 });
    for (let i = 0; i < 20; i++) data.push({ layer: 2, color: 'orange', width: 50 + Math.random() * 50 });
    for (let i = 0; i < 15; i++) data.push({ layer: 3, color: 'gold', width: 50 + Math.random() * 50 });
    //for (let i = 0; i < 10; i++) data.push({ layer: 4, color: "lightgreen", width: 50 + Math.random()*50 });
    //for (let i = 0; i < 5; i++) data.push({ layer: 5, color: "lightblue", width: 50 + Math.random()*50 });
  ...
  function doubleCircleLayout(diagram) {
    ...
    while (((nodes = nodesByLayer(diagram, layer)), nodes.count > 0)) {
      ...
      // recenter at (0, 0)
      const cntr = layout.actualCenter;
