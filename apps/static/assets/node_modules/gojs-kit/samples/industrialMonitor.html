...
<div id="sample">
  ...
  <p>
    ...
    monitoring panels, and simulated data is updated periodically. GoJS is a common choice for industries
    wishing to make SCADA diagrams, Manufacturing Operations Management diagrams, process flows, and more.
  </p>
  <p>
...
<script id="code">
  function init() {
    ...
    myDiagram.nodeTemplateMap.add('',
      ...
      new go.Node('Spot', { itemTemplate: tankPort })
      ...
      new go.Node('Auto')
        ...
        .add(
          ...
          new go.TextBlock({
            ...
            .bind('margin', 'direction',
              d => d === 'right' ? new go.Margin(8, 40, 8, 8) : new go.Margin(8, 8, 8, 40))
            .bind('alignment', 'direction',
              d => d === 'right' ? new go.Spot(0.3, 0.5) : new go.Spot(0.7, 0.5))
            .bind('text')
        ));
    ...
    const valuesTable =
      new go.Panel('Table', { itemTemplate: valuesTableItem })
        .bind('itemArray', 'values');

    // This is a component of the "monitor" node template, showing a dynamic number of status blocks
    const statusPanelTemplate =
    new go.Panel('Spot')
      .add(
        new go.Shape({ width: 18, height: 18, fill: colors.white })
          .bind('fill'),
        new go.TextBlock()
          .set(textDefaults)
          .bind('text')
    ...
    setInterval(() => {
      ...
      myDiagram.commit(() => {
        const controlNodes = ['cTCV102', 'cFCV101', 'cFM102', 'cFM103']
            .map(k => myDiagram.findNodeForKey(k));
