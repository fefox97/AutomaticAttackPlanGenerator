...
<script id="code">
  ...
  const GridCellHeight = 20; // document units; cannot be changed dynamically
  let GridCellWidth = 12; // document units per day; this can be modified -- see rescale()
  const TimelineHeight = 24; // document units; cannot be changed dynamically
  ...
  const myTasksHeader =
    new go.Part('Table', { // the timeline at the top of the myTasks viewport
        layerName: 'Adornment',
        pickable: false,
  ...
  const myTimeline =
    new go.Part('Graduated', { // the timeline at the top of the myGantt viewport
        layerName: 'Adornment',
        pickable: false,
        position: new go.Point(-26, 0), // position will be set in "ViewportBoundsChanged" listener
  ...
  const myGrid =
    new go.Part('Grid', { // the grid of horizontal lines
        layerName: 'Grid',
        pickable: false,
  ...
  const myHighlightDay =
    new go.Part({ // the vertical highlighter covering the day where the mouse is
      layerName: 'Grid',
      visible: false,
      pickable: false,
  ...
  const myHighlightTask =
    new go.Part({ // the horizontal highlighter covering the current task
      layerName: 'Grid',
      visible: false,
      pickable: false,
