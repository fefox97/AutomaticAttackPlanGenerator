...
export class WallReshapingTool extends go.Tool {
  ...
  private maybePerformWallSplit(w: go.Group, ip: go.Point): void {
    ...
    walls.iterator.each(function (wc: go.Group) {
      ...
      if (
        (tool.pointsApproximatelyEqual(s, ws) && tool.pointsApproximatelyEqual(ip, we)) ||
        (tool.pointsApproximatelyEqual(s, we) && tool.pointsApproximatelyEqual(ip, ws))
      ) {
        alreadyExists = true;
      }
      if (
        (tool.pointsApproximatelyEqual(ip, ws) && tool.pointsApproximatelyEqual(e, we)) ||
    ...
    if (l1 > 1 && l2 > 1 && !alreadyExists) {
        ...
        rooms.iterator.each(function (r: go.Node) {
          ...
          for (let i: number = 0; i < bw.length; i++) {
            ...
            if (wk === w.data.key) {
              ...
              const nonIpEndpoint: go.Point = tool.pointsApproximatelyEqual(rw.data.startpoint, ip)
                ? rw.data.endpoint
                : rw.data.startpoint;
              ...
              if (isConnectedToBounds) {
                ...
                const w1NonIpEndpoint: go.Point = tool.pointsApproximatelyEqual(
                  w1.data.startpoint,
                  ip
                )
                  ? w1.data.endpoint
                ...
                iw2.iterator.each(function (ww: go.Group) {
                  for (let j: number = 0; j < bw.length; j++) {
                    ...
                    if (ww.data.key === wk2 && w1.data.key !== ww.data.key) {
                      ...
                      const ww2OtherEndpoint: go.Point = tool.pointsApproximatelyEqual(
                        ww2.data.startpoint,
                        w1NonIpEndpoint
                      )
                        ? ww2.data.endpoint
      ...
      const w1op: go.Point = tool.pointsApproximatelyEqual(w1.data.startpoint, ip)
        ? w1.data.endpoint
        : w1.data.startpoint;
      const w2op: go.Point = tool.pointsApproximatelyEqual(w2.data.startpoint, ip)
        ? w2.data.endpoint
        : w2.data.startpoint;
