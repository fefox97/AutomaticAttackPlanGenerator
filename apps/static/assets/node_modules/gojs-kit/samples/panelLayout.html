...
<script id="code">
  ...
  // Register the PanelLayout so that go.GraphObject.build can use it by name:
  ...
  function init() {
    ...
    myDiagram.nodeTemplate =
      new go.Node('Auto', { width: 100, height: 100 })
        .add(
          new go.Shape({ fill: 'transparent', strokeWidth: 2 })
            .bind('stroke', 'color'),
          new go.Panel('Cascading', {
              itemTemplate:
                new go.Panel()
                  .add(
                    new go.Shape({ width: 20, height: 20, strokeWidth: 0 })
                      .bind('figure', 'fig')
                      .bind('fill', 'color')
                  )
            })
