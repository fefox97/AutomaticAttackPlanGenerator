...
<meta property="og:url" content="https://gojs.net/latest/intro/overview.html" />
<meta property="twitter:url" content="https://gojs.net/latest/intro/overview.html" />
...
<p>
  An <a>Overview</a> is a subclass of <a>Diagram</a> that is used to display all of the <a>Part</a>s of another diagram and to show where that diagram's
  viewport is relative to all of those parts. The user can also scroll the overviewed diagram by clicking or dragging within the overview.
</p>
<p>
  The initialization of an <a>Overview</a> is just a matter of setting <a>Overview.observed</a> to refer to the <a>Diagram</a> that you want it to show. So
  there needs to be a DIV for your main diagram, for which you create a Diagram in the normal manner, and a separate DIV for your overview, for which you create
  the Overview in a very simple manner.
</p>
<p>See samples that make use of <a>Overview</a>s in the <a href="../samples/index.html#overview">samples index</a>.</p>
<p>The code below first creates a Diagram that we want to view. It initializes the diagram with 1000 nodes of random colors.</p>
<p>
  It then creates an <a>Overview</a> and sets <a>Overview.observed</a> to the above Diagram. The DIV for the overview is named "myOverviewDiv". You can, if you
  wish, set <a>Overview.observed</a> at a later time. You can also set it to null in order to have the Overview stop showing any Diagram.
</p>
...
<p>Animations are not shown in Overviews. Rendering images or SVG does not work for Overviews.</p>
