...
<meta itemprop="name" content="Making Images" />
<meta property="og:title" content="Making Images" />
<meta name="twitter:title" content="Making Images" />
...
<meta property="og:url" content="https://gojs.net/latest/intro/makingImages.html" />
<meta property="twitter:url" content="https://gojs.net/latest/intro/makingImages.html" />
...
<p>
  <b>GoJS</b> has two functions for creating raster images: <a>Diagram.makeImageData</a>, which outputs a Base64 image data string, and
  <a>Diagram.makeImage</a>, which is a convenience function that calls <a>Diagram.makeImageData</a> and returns a new HTMLImageElement with the image data as
  its source. Both functions have the same single argument, a JavaScript Object that contains several definable properties, enumerated in the documentation.
</p>
<p>
...
<hr />
<p>Calling makeImage with no arguments or with an empty properties object results in an image that is the same size as the Diagram's viewport.</p>
...
<hr />
<p>
  Calling makeImage with an object that has the "scale" property set to 1 results in an image that includes the whole diagram, not just the area visible in the
...
<p>
  The following image is created by setting the size option of makeImage. Note that the canvas is scaled uniformly and any extra space is placed on the bottom
...
<p>
  The following image is also created by setting the size option of makeImage, but only the width is set. The height will be whatever size is needed to
  uniformly contain the Diagram.
</p>
...
<p>
  We can set the type and details properties of the argument object in order to retrieve different kinds of images. The only widely supported type is
  "image/jpeg". The details for a jpeg determine its quality by using values from 0 to 1 inclusive. Jpegs are not commonly used for Diagrams because their lossy
  compression can render text unreadable.
</p>
<p>
  The following image is an outputted jpeg. Note how the transparent background is turned black, because the jpeg format does not support alpha transparency,
  and the default state of the HTML5 canvas is that of fully transparent black pixels, rgba(0,0,0,0).
</p>
...
<p>The following image is a jpeg created with an AntiqueWhite background specified.</p>
...
<p>The following image is a jpeg created (with an AntiqueWhite background) and the details option, at very low quality.</p>
...
<p>
  You do not need to involve the web server if you want the user to download an image. See the sample
  <a href="../samples/minimalBlob.html" target="_blank">Minimal Blob</a>. Note that that sample only downloads a single image.
</p>
<p>
  We suggest that you use SVG for downloading an image, if that choice is acceptable to your users. That sample is at
  <a href="../samples/minimalSvg.html" target="_blank">Minimal SVG</a>.
</p>
