...
<meta property="og:url" content="https://gojs.net/latest/intro/layers.html" />
<meta property="twitter:url" content="https://gojs.net/latest/intro/layers.html" />
...
<p>
  Every Diagram starts off with several standard layers. These are their <a>Layer.name</a>s, in order from furthest behind to most in front:
</p>
<ul>
...
<pre class="lang-js" id="layers"><code>
  ...
  diagram.addLayerBefore(new go.Layer({ name: "blue" }), forelayer);
  diagram.addLayerBefore(new go.Layer({ name: "green" }), forelayer);
  diagram.addLayerBefore(new go.Layer({ name: "orange" }), forelayer);

  diagram.nodeTemplate =  // no links or grouping, so can use the simpler Part class
    new go.Part("Spot", {
        selectionChanged: p => {
      ...
      .bind("location", "loc")
      .add(
        new go.Shape({ width: 80, height: 80 })
          .bind("fill", "color"),
        new go.TextBlock({ stroke: "white", font: "bold 12px sans-serif" })
      );
  ...
  toggleVisible = (layername, e) => {
    diagram.commit(d => {
      const layer = d.findLayer(layername);
    ...
    new go.Part("Spot")
      ...
      .add(
        new go.Shape({
            ...
            stroke: 'rgb(50,50,50)', fill: 'rgb(50,100,255)'
          }),
        new go.TextBlock({ font: "52px sans-serif", stroke: 'whitesmoke' })
          .bind("text", "zOrder"),
        go.GraphObject.build("Button", {
          ...
          .add(
            new go.Shape("LineH", { width: 14, height: 14 })
          ),
        go.GraphObject.build("Button", {
          ...
          .add(
            new go.Shape("PlusLine", { width: 14, height: 14 })
          )
      );

  const array = [];
  for (let i = 0; i < 12; i++) {
    const data = { loc: new go.Point(Math.random()*500, Math.random()*200) };
