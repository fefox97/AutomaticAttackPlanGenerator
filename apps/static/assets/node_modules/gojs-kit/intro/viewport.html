...
<meta itemprop="name" content="Coordinate Systems" />
<meta property="og:title" content="Coordinate Systems" />
<meta name="twitter:title" content="Coordinate Systems" />
...
<meta property="og:url" content="https://gojs.net/latest/intro/viewport.html" />
<meta property="twitter:url" content="https://gojs.net/latest/intro/viewport.html" />
...
<p>
  All coordinate systems in <b>GoJS</b> have <a>Point</a>s with increasing values of X going rightwards and increasing values of Y going downwards.
</p>
...
<p>
  ...
  These document coordinates are unchanging when the user scrolls or zooms the diagram.
</p>
...
<p>
  ...
  The range of the document bounds might be entirely positive x and y values,
  or that might be entirely negative, or more likely they may cover both negative and positive values.
</p>
<p>
  But a Part with a <a>Part.location</a> of (0, 0) in document coordinates is not always drawn
  at the top-left corner of the HTML Div element that the user sees in the page.
...
<p>
  ...
  Scrolling and panning change the <a>Diagram.position</a>.
  Zooming in or out changes the <a>Diagram.scale</a>.
  ...
  However very few properties or method arguments or return values are in view coordinates --
  almost everything is in document coordinates or in panel coordinates.
</p>
<p>
...
<pre class="lang-js" id="diffCoordSystems" style="display: none"><code>
  ...
  // colors
  const docGridStroke = "rgba(70, 130, 180, 0.5)";
  const viewGridStroke = "rgba(255, 128, 128, 1)";
  const commentStroke = "brown";

  const cellSide = 20; // side length of one grid cell
  const cellSize = new go.Size(cellSide * 5, cellSide * 5);
  const pointSize = 7;
    ...
    $(go.Node, "Vertical",
      ...
      new go.Binding("location", "point", function gridPointLocation(point) {
          ...
          textLines.forEach(text => {
            const textBlock = $(go.TextBlock, { text: text, font: "bold 10pt sans-serif"});
          ...
          const textBlock = $(go.TextBlock, { text: label, font: "bold 10pt sans-serif" });
...
<script>
  goCode('diffCoordSystems', 750, 650);
</script>
...
<pre class="lang-js" id="coordsystems"><code>
  ...
  diagram.nodeTemplate =
    new go.Part({  // no links or grouping, so use the simpler Part class instead of Node
      ...
      .add(
        new go.Shape("PlusLine", { name: "SHAPE", width: 8, height: 8 }),
        new go.TextBlock({ position: new go.Point(6, 6), font: "8pt sans-serif" })
          .bind("text", "loc")
      );
...
<script>
  goCode('coordsystems', 300, 150);
</script>
<textarea id="Message1" style="font-size: 11px; width: 300px; height: 70px">(move mouse over node to see points in document and in view coordinates)</textarea>
...
<p>
  ...
  If some or all of the parts are far apart from each other, the document bounds might be large,
  even if there are only two parts or if there is just one really large part.
  The <a>Diagram.documentBounds</a> value is independent of the <a>Diagram.viewportBounds</a>.
  The former only depends on the bounds of the parts; the latter only depends on the size of the canvas and the diagram's position and scale.
</p>
<p>
...
<p>
  The diagram does not compute a new value for <a>Diagram.documentBounds</a> immediately
  upon any change to any part or the addition or removal of a part.
  Thus the <a>Diagram.documentBounds</a> property value may not be up-to-date until after a transaction completes.
</p>
<p>
  The relative sizes of the <a>Diagram.documentBounds</a> and <a>Diagram.viewportBounds</a> control whether or not scrollbars are needed.
  You can set <a>Diagram.hasHorizontalScrollbar</a> and/or <a>Diagram.hasVerticalScrollbar</a> to false to make sure no scrollbar appears even when needed.
  Or set <a>Diagram.scrollMode</a> to <a>ScrollMode.Infinite</a>.
</p>
<p>
...
<p>
  ...
  he long or "page" scrolling distance is controlled by the size of the viewport.
  If you want to control the precise values that the <a>Diagram.position</a> may have, you can specify a
  <a>Diagram.positionComputation</a> function. See the example below.
</p>
<p>
...
<ul>
  ...
  <li>calling <a>CommandHandler.decreaseZoom</a>, <a>CommandHandler.increaseZoom</a>, <a>CommandHandler.resetZoom</a>, or <a>CommandHandler.zoomToFit</a></li>
</ul>
<p>
  ...
  If you want to control the precise values that the <a>Diagram.scale</a> may have, you can specify a <a>Diagram.scaleComputation</a> function.
...
<p>
  If you want a Part to always be displayed within the viewport, no matter how the user scrolls or zooms,
  you can add them to a Layer that has <a>Layer.isViewportAligned</a> set to true.
  Such Parts are positioned by the <a>GraphObject.alignment</a> and <a>GraphObject.alignmentFocus</a> properties
  rather than by the <a>GraphObject.position</a> or <a>Part.location</a> properties.
  Read more at <a href="legends.html#StaticParts">Static Parts</a>.
</p>
...
<p>
  ...
  This can be useful when users need extra space at the edges of a Diagram, for instance to
  have an area to create new nodes with the <a>ClickCreatingTool</a>.
</p>
<p>
  <a>Diagram.padding</a> is added as if part of the document bounds,
  whereas <a>Diagram.scrollMargin</a> makes sure you can scroll to empty space beyond the document bounds.
  Because of this, <a>Diagram.scrollMargin</a> does not create additional scrollable empty space if none is needed to scroll the margin
  distance beyond, such as when the document bounds are very small in the viewport.
</p>
<p>
  Below is a Diagram with <a>Diagram.scrollMargin</a> set to <code>100</code>.
...
<pre class="lang-js" id="scrollmargin" style="display: none"><code>
  diagram.grid =
    new go.Panel("Grid")
      .add(
        new go.Shape("LineH", { stroke: "gray", strokeWidth: 0.5 }),
        new go.Shape("LineH", { stroke: "darkslategray", strokeWidth: 1.5, interval: 10 }),
        new go.Shape("LineV", { stroke: "gray", strokeWidth: 0.5 }),
...
<script>
  goCode('scrollmargin', 400, 400);
</script>

<h2 id="ScrollingModes">Scrolling modes</h2>
<p>
  <a>Diagram.scrollMode</a> allows the user to either scroll to document bound borders with <a>ScrollMode.Document</a> (the default),
  or scroll endlessly with <a>ScrollMode.Infinite</a>.
</p>
<p>
  <a>Diagram.positionComputation</a> and <a>Diagram.scaleComputation</a> allow you to determine what positions and scales are acceptable to be scrolled to.
...
<pre class="lang-js"><code>
  function positionfunc(diagram, pos) {
    const size = diagram.grid.gridCellSize;
    return new go.Point(
...
<script type="text/javascript">
  function positionfunc(diagram, pos) {
    const size = diagram.grid.gridCellSize;
    return new go.Point(Math.round(pos.x / size.width) * size.width, Math.round(pos.y / size.height) * size.height);
  }
  ...
  const infscroll = document.getElementById('infscroll');
  infscroll.addEventListener('change', e => {
    myDiagram2.commit(d => {
      d.scrollMode = infscroll.checked ? go.ScrollMode.Infinite : go.ScrollMode.Document;
    });
  });

  const poscomp = document.getElementById('poscomp');
  poscomp.addEventListener('change', e => {
    myDiagram2.commit(d => {
      d.positionComputation = poscomp.checked ? positionfunc : null;
    });
  });

  const scalecomp = document.getElementById('scalecomp');
  scalecomp.addEventListener('change', e => {
    myDiagram2.commit(d => {
      d.scaleComputation = scalecomp.checked ? scalefunc : null;
    });
  });
</script>
...
<p>
  A <a>GraphObject</a> that is not a <a>Part</a> but is an element of a <a>Panel</a> has measurements that are in panel coordinates,
  not in document coordinates.
  That means that <a>GraphObject.position</a>, <a>GraphObject.actualBounds</a>, <a>GraphObject.maxSize</a>, <a>GraphObject.minSize</a>,
  <a>GraphObject.measuredBounds</a>, <a>GraphObject.margin</a>, and <a>RowColumnDefinition</a> properties
  apply to all elements of a panel using the same coordinate system.
</p>
<p>
  Some <a>GraphObject</a> properties use units that have values before they are transformed for use by the containing <a>Panel</a>'s coordinate system.
  In particular, <a>GraphObject.desiredSize</a> (which means <a>GraphObject.width</a> and <a>GraphObject.height</a>), <a>GraphObject.naturalBounds</a>,
  <a>Shape.geometry</a>, and <a>Shape.strokeWidth</a> are in "local" coordinates,
...
<p>
  ...
  If you want to get the document position of some object that is within a Node, call <a>GraphObject.getDocumentPoint</a>.
</p>
<p>
...
<pre class="lang-js" id="nestedpanelcoords" style="display: none"><code>
  ...
  function prec(conv) { return g => conv(g).toPrecision(3); }

  // generates cells in data tables
  function dataBlock(conv, alo1, alo2) {
    return $(go.TextBlock,
      new AlignmentObject(alo1, alo2),
      new go.Binding("text", "gro", prec(conv)));
  }
    ...
    $(go.Node, "Auto",
      ...
      $(go.Panel, "Table",
        ...
        $(go.Panel, "TableRow", {row: 1},
          ...
          // container angle
          dataBlock(g => g.angle, 1, 2),
          // document angle
          dataBlock(g => g.getDocumentAngle(), 3, 2)),

        // scale row
        $(go.Panel, "TableRow", {row: 2},
          $(go.TextBlock, "scale", {column: 0}),
          // container scale
          dataBlock(g => g.scale, 1, 2),
          // document scale
          dataBlock(g => g.getDocumentScale(), 3, 2)),
        ...
        $(go.Panel, "TableRow", {row: 3},
          ...
          dataBlock(g => g.actualBounds.x, 1, 1),
          dataBlock(g => g.actualBounds.y, 2, 1),

          // document x and y values
          dataBlock(g => g.getDocumentBounds().x, 3, 1),
          dataBlock(g => g.getDocumentBounds().y, 4, 1)),
        ...
        $(go.Panel, "TableRow", {row: 4},
          ...
          dataBlock(g => g.actualBounds.width, 1, 1),
          dataBlock(g => g.actualBounds.height, 2, 1),

          // document width and height
          dataBlock(g => g.getDocumentBounds().width, 3, 1),
          dataBlock(g => g.getDocumentBounds().width, 4, 1)))));
    ...
    $(go.Panel, "Vertical",
      ...
      $(go.Panel, "Position",
        ...
        $(go.Panel, "Auto", { position: new go.Point(25,0), desiredSize: new go.Size(60,90)},
          new go.Shape("Triangle", { fill: "transparent" }),
      ...
      $(go.Panel, "Spot",
        ...
        new go.Shape("RoundedRectangle", {strokeWidth: 0, desiredSize: new go.Size(50,100), fill: "transparent"}),
...
<script>
  goCode('nestedpanelcoords', 600, 400);
</script>
<p>
...
<p>
  ...
  But because the "Vertical" Panel has a scale of 1.5, its effective scale is 1.2 overall, the product of those individual scales (0.8 x 1.5).
</p>
